    ...
    switch (type) {
        ...
        case "search":
            moviesSearchResultList = results.map(elem => {
                ...
                return <MoviesResultsItem
                    key={elem.imdbID} id={elem.imdbID} isSelectedMovie={isSelectedMovie}
                    onClick={() => updateCurrentMovie(elem.imdbID, "", elem.Title, elem.Year)}
                    poster={elem.Poster} hasValidPoster={hasValidPoster} title={elem.Title} year={elem.Year}
                    onTrailerClick={e => { e.stopPropagation(); toggleWatchTrailer(`${elem.Title} ${elem.Year}`, elem.imdbID); }}
                />
            });
            break;
        ...
        case "trending":
            moviesSearchResultList = results.map(elem => {
                ...
                return <MoviesResultsItem
                    key={elem.id} isSelectedMovie={isSelectedMovie}
                    onClick={() => { getIMDBID(elem.id, elem.title, year); }}
                    poster={POSTER_BASE_URL + elem.poster_path} hasValidPoster={hasValidPoster} title={elem.title} year={year}
                    onTrailerClick={e => { e.stopPropagation(); toggleWatchTrailer(`${elem.title} ${year}`, elem.id); }}
                />
            });
            break;
